...
/**
 * Stores the OAuth2 authorization request in a short-lived HTTP-only cookie
 * instead of the HTTP session. This is required because the app uses
 * STATELESS session management (JWT), so HttpSession is never created.
 *
 * Without this, Spring Security throws "Authorization Request not found"
 * on the callback because the state parameter cannot be retrieved.
 */
@Component
